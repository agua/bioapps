...
=head2
	...
	PURPOSE
		...
		1. SCROLL ALTERNATELY THROUGH QUERY AND TARGET FILE, DEPENDING ON
			...
			WHICH HAS THE MOST UPSTREAM START POINT, UNTIL THE FEATURES
			
			OVERLAP
		
		2. PRINT QUERY LINES TO OUTPUT FILE WHILE QUERY LINES OVERLAP OR
		...
		4. REPEAT 1-3 UNTIL REACHED THE END OF EITHER FILE
...
my $DEBUG = 1;
	...
	print "Filter::scrollMatch    Filter::scrollMatch(args)\n" if $DEBUG;
	print "Filter::scrollMatch    args:\n" if $DEBUG;
	...
	#print "Filter::scrollMatch    complement: $complement\n";
	#print "Filter::scrollMatch    flanks: $flanks\n";
	...
	while ( $doing )
	{
		...
		print "\n\n******** Doing $doingcounter  **  Filter::scrollMatch\n" if $DEBUG;
		...
		print "Filter::scrollMatch    Doing query : ", $query->toString(), "\n" if $DEBUG;
		print "Filter::scrollMatch    Doing target: ", $target->toString(), "\n" if $DEBUG;
		
		my $targetUpstream = $target->upstream($query);
		print "Filter::scrollMatch    targetUpstream: $targetUpstream\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "Filter::upstream    Filter::upstream(targetchromosome, querychromosome)\n" if $DEBUG;
	print "Filter::upstream    targetchromosome: $targetchromosome\n" if $DEBUG;
	print "Filter::upstream    querychromosome: $querychromosome\n" if $DEBUG;
	print "Filter::upstream    targetstart: $targetstart\n" if $DEBUG;
	print "Filter::upstream    querystart: $querystart\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "Filter::upstreamChromosome    Filter::upstreamChromosome(targetchromosome, querychromosome)\n" if $DEBUG;
	print "Filter::upstreamChromosome    targetchromosome: $targetchromosome\n" if $DEBUG;
	print "Filter::upstreamChromosome    querychromosome: $querychromosome\n" if $DEBUG;
	...
	print "Filter::upstreamChromosome    returning target_upstream: $target_upstream\n" if $DEBUG;
...
sub scroll
{
    ...
    #print "Filter::scroll    Filter::scroll(filehandle, matcher, type, chromosome, start,stop)\n";    
    #print "Filter::scroll    type: $type, chromosome: $chromosome, start: $start, stop: $stop\n";    
    ...
    while ( $current_start < $start and $current_stop < $start )
    {
        ...
        #print "Filter::scroll    Scrolling to start position: $start because current_stop: $current_stop\n";
		$line = <$filehandle>;
...
{
	#print "Filter::scrollMatchRegex    \@_: \n";
	#print Dumper @_;
	...
	#print "Filter::scrollMatchRegex    Filter::scrollMatchRegex(type, indexes)\n";
	#print "Filter::scrollMatchRegex    type: $type\n";
	#print "Filter::scrollMatchRegex    indexes: $indexes\n" if defined $indexes;
	...
	return sub {
		return if not @_;
		my $input = shift;
		#print "Filter::scrollMatchRegex sub    dbsnp\n";
		#print "Filter::scrollMatchRegex sub    input: $input\n";

		$input =~ /^\S+\tchr(\S+)\t(\d+)\t(\d+)/;
		#print "Filter::scrollMatchRegex sub    \$1: $1, \$2: $2, \$3: $3\n";
...
sub value
{
	...
	#print "Parameter $parameter has value: ";print Dumper $value;
...
sub AUTOLOAD {
	...
	print "++++ Filter::AUTOLOAD(self, newvalue)\n" if $DEBUG;
