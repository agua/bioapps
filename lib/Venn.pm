...
#print "Venn::collateSimple    outputs:\n";
...
sub collateBins {
	...
	foreach my $replique ( @$repliques )
	{
		...
		foreach my $bin ( @$bins )
		{
			...
			$filestub =~ s/\.bam//;
			my $prefix = $filestub . ".";		
			#print "Venn::collateBins    binfile: $binfile\n";
			#print "Venn::collateBins    prefix: $prefix\n";
			...
			#print "Venn::collateBins    queryfile: $queryfile\n";
			#print "Venn::collateBins    targetfile: $targetfile\n";

			if ( not -f $queryfile )
			{
				#print "Venn::collateBins    Can't find either inputfile. Skipping\n" if not -f $targetfile;
...
#			print "Venn::collateBins    queryonly: $queryonly\n";
			#print "Venn::collateBins    targetonly: $targetonly\n";
			#print "Venn::collateBins    both: $both\n";
			$queryonly_count 	+= $self->lineCount($queryonly);
			$targetonly_count 	+= $self->lineCount($targetonly);
			$both_count 		+= $self->lineCount($both);

			#print "Venn::collateBins    $both $both_count\n";
		}

		#print "queryonly_count: $queryonly_count\n";
		#print "targetonly_count: $targetonly_count\n";
		#print "both_count: $both_count\n";
...
#print "Venn::collateBins    outputs:\n";
#print Dumper $outputs;
#exit;
...
#my $DEBUG = 1;
	...
	print "Venn::gtfSummary    Venn::gtfSummary(line)\n" if $DEBUG;
	print "Venn::gtfSummary    line: $line\n" if $DEBUG;
	print "Venn::gtfSummary    chromosome: $chromosome\n" if $DEBUG;
	print "Venn::gtfSummary    start: $start\n" if $DEBUG;
	print "Venn::gtfSummary    stop: $stop\n" if $DEBUG;
	print "Venn::gtfSummary    id: $id\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "Venn::filterTranscripts    Venn::filterTranscripts()\n" if $DEBUG;
	print "Venn::filterTranscripts    inputfile: $inputfile\n" if $DEBUG;
	print "Venn::filterTranscripts    outputfile: $outputfile\n" if $DEBUG;
	...
	print "Venn::filterTranscripts    outputfile printed:\n\n$outputfile\n\n" if $DEBUG;
...
print "Venn::match    Venn::match(queryfile, targetfile)\n" if $DEBUG;
print "Venn::match    queryfile: $queryfile\n" if $DEBUG;
print "Venn::match    targetfile: $targetfile\n" if $DEBUG;
	...
	while ( <QUERY> )
	{
		...
		print "Venn::match    queryposition: $queryposition\n" if $DEBUG;
		my $match = 0;
		foreach my $target ( @queries )
		{
			my ($targetposition) = $target =~ /^\S+\s\S+\s+\S+\s+(\S+)/;
			print "Venn::match    targetposition: $targetposition\n" if $DEBUG;
			print "Venn::match    MATCHED\n" if $targetposition == $queryposition and $DEBUG;
...
sub AUTOLOAD {
	...
	print "++++ Venn::AUTOLOAD(self, newvalue)\n" if $DEBUG;
