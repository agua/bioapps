...
sub convert_hash {
	...
	#print "++++ convert_hash(hash, mapping)\n";
	#print "Hash:\n";
	#print Dumper $hash;
	...
	foreach my $key ( keys %$hash )
	{
		...
		if ( not exists $mapping->{$key} )	{	next;	}	#print "****Key $key not found!!\n";next;	}
...
sub input_range {
	...
	while ( length($input_string) > 0 )
	{
		...
		{
#			print "Pushing $1 into input_range\n";
			push @$input_range, $1;
			$input_string 	=~ s/^[^\-^,]+,*//;
			#$input_string =~ s/^,//g;
		}
...
sub files_by_pattern {
	...
	#print "++++ Util::files_by_pattern(files, pattern, exclude)\n";
	#print "Files: @$files\n";
...
#    print "DIRECTORY (files_in_subs): $directory\n";
...
sub split_fasta {
	...
	#print "Sequence record: $sequence_record\n";
...
#		print "$input_string\n";
		...
		{
#			print "Pushing $1 into input_array\n";
			push @$input_array, $1;
			$input_string 	=~ s/^\d+//;
			$input_string =~ s/^,//g;
		}
...
sub block_text_array {
	...
	if ( not defined $block_width )	{	print "Util::block_text_array: Block width not defined\n";	exit;	}
	if ( not defined $split)	{	print "Util::block_text_array: Split (word | char) not defined\n";	exit;	}
	...
	if ( $split =~ /^word$/ )
	{
		...
		while ( @word_array )
		{
			...
			#print "Length of word_array[0]: " , length($word_array[0]), "\n";
			...
			#print "push \@\$block_text_array, $line\n";
			#
			#print "here\n";
			...
			{				
				...
				{
					#print "Splitting at comma\n";
					#print "Before comma: $1\n";
					#print "After comma: $2\n";
				}
	...
	#print "block_text_array: \n";
	#print Dumper $block_text_array;
