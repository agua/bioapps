...
sub generateChunkInputfiles {
	...
	print "SNP::generateChunkInputfiles    start: $start\n" if defined $start and $DEBUG;
	print "SNP::generateChunkInputfiles    samtools: $samtools\n" if $DEBUG;
	print "SNP::generateChunkInputfiles    samtools_index: $samtools_index\n" if $DEBUG;
	
#my $DEBUG = 1;
	print "SNP::generateChunkInputfiles    SNP::generateChunkInputfiles(chromosome, inputdirs, outputdir, filename, binlevel, bindir, maxdepth)\n" if $DEBUG;

	#### GET CHROMOSOMES
	my $inputdir = $$inputdirs[0];
	my $chromosomes = $self->getChromosomes($inputdir);
	print "SNP::generateChunkInputfiles    inputdirs: @$inputdirs\n" if $DEBUG;
	print "SNP::generateChunkInputfiles    chromosomes: @$chromosomes\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		foreach my $chromosome ( @$chromosomes )
		{
			...
			print "SNP::generateChunkInputfiles    bamfile: $bamfile\n" if $DEBUG;
			...
			foreach my $bin ( @$bins )
			{
				...
				print "SNP::generateChunkInputfiles    outdir: $outdir\n" if $DEBUG;
				...
				{
					print "SNP::generateChunkInputfiles    bin->{number} not defined for bin:\n" if $DEBUG;
					print Dumper $bin if $DEBUG;
					exit;
				}
				...
				@$jobs = (@$jobs, @$chunkjobs);
				#print "SNP::setChunkJobs    No. chunkjobs: ", scalar(@$chunkjobs), "\n";
				#print "SNP::setChunkJobs    chunkjobs:\n";
				#print Dumper $chunkjobs;
			
				#### SET OUTPUT FILE			
				my $snpfile = "$outputdir/$chromosome/$filestub.snp";
				$outfile_chunks->{$snpfile} = $chunkfiles;
				#print "SNP::setChunkJobs    outfile_chunks:\n";
...
sub binBamToSnp {
	...
	my $samtools_index 	=	$self->get_samtoolsindex();
	my $start 			=	$self->get_start();
	print "SNP::binBamToSnp    start: $start\n" if defined $start and $DEBUG;
	print "SNP::binBamToSnp    samtools: $samtools\n" if $DEBUG;
	print "SNP::binBamToSnp    samtools_index: $samtools_index\n" if $DEBUG;
	
#my $DEBUG = 1;
	print "SNP::binBamToSnp    SNP::binBamToSnp(chromosome, inputdirs, outputdir, filename, binlevel, bindir, maxdepth)\n" if $DEBUG;

	#### GET CHROMOSOMES
	my $inputdir = $$inputdirs[0];
	my $chromosomes = $self->getChromosomes($inputdir);
	print "SNP::binBamToSnp    inputdirs: @$inputdirs\n" if $DEBUG;
	print "SNP::binBamToSnp    chromosomes: @$chromosomes\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		foreach my $chromosome ( @$chromosomes )
		{
			#### INPUTFILE AND OUTDIR
			my $bamfile = "$inputdir/$chromosome/$filename";
			print "SNP::binBamToSnp    bamfile: $bamfile\n" if $DEBUG;
			...
			foreach my $bin ( @$bins )
			{
				#### SET OUTPUT DIR
				my $outdir = "$outputdir/$dirname/$chromosome";
				print "SNP::binBamToSnp    outdir: $outdir\n" if $DEBUG;
				File::Path::mkpath($outdir) if not -d $outdir;
				...
				if ( not defined $binnumber )
				{
					print "SNP::binBamToSnp    bin->{number} not defined for bin:\n" if $DEBUG;
					print Dumper $bin if $DEBUG;
					exit;
				}
				my $chunkdir = "$outputdir/$chromosome/indir$dirname/bin$binnumber";
...
#exit;
				...
				print "SNP::binBamToSnp    chunkjobs:\n" if $DEBUG;
				print "SNP::binBamToSnp    chunkjobs[0]: \n" if $DEBUG;
				print Dumper $$chunkjobs[0] if $DEBUG;
				#print join "\n", @$chunkjobs;
				...
				my $snpfile = "$outputdir/$chromosome/$filestub.snp";
				$outfile_chunks->{$snpfile} = $chunkfiles;
				#print "SNP::binBamToSnp    outfile_chunks: \n";
				#print Dumper $outfile_chunks;
...
#my $DEBUG = 1;
	print "SNP::printChunkfileJobs    SAV::printChunkfileJobs(chunksize, inputfile, outputdir)\n" if $DEBUG;	
	print "SNP::printChunkfileJobs    chunksize: $chunksize\n" if $DEBUG;
	print "SNP::printChunkfileJobs    inputfile: $inputfile\n" if $DEBUG;
	print "SNP::printChunkfileJobs    outputdir: $outputdir\n" if $DEBUG;

	#### SET CHUNKFILE
	my ($filestub, $suffix) = $inputfile =~ /\/([^\/]+)\.([^\.]+)$/;
	print "SNP::printChunkfileJobs    filestub: $filestub\n" if $DEBUG;
	print "SNP::printChunkfileJobs    suffix: $suffix\n" if $DEBUG;
	...
	print "SNP::printChunkfileJobs    chromosome: $chromosome\n" if $DEBUG;
	print "SNP::printChunkfileJobs    binstart: $binstart\n" if $DEBUG;
	print "SNP::printChunkfileJobs    binstop: $binstop\n" if $DEBUG;

	my $jobs = [];
	my $chunkfiles = [];
	my $chunknumber = 1;
	my $chunkfile = "$outputdir/$filestub.chunk$chunknumber.$suffix";
	print "SNP::printChunkfileJobs    chunkfile: $chunkfile\n" if $DEBUG;
	my $start = 0;
	my $stop = 0;
	while ( $stop < $binstop )	
	{
		...
		print "SNP::printChunkfileJobs    $command\n" if $DEBUG;
	...
	#print "SNP::printChunkfileJobs    chunkfiles:\n" if $DEBUG;
	#print join "\n", @$chunkfiles;
	#print "\n";

	##### RUN JOBS
	#print "\nSNP::printChunkfileJobs    Running jobs\n";
...
#my $DEBUG = 1;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		foreach my $chromosome ( @$chromosomes )
		{
			...
			#print "SNP::binBamToSnp    outdir: $outputdir\n" if $DEBUG;
			...
			#print "Can't create outdir: $outputdir\n" if not -d $outputdir;
...
#my $DEBUG = 1;
	print "SNP::getChunkfiles    SAV::getChunkfiles(chunksize, inputfile, outputdir)\n" if $DEBUG;	
	print "SNP::getChunkfiles    chunksize: $chunksize\n" if $DEBUG;
	print "SNP::getChunkfiles    inputfile: $inputfile\n" if $DEBUG;
	print "SNP::getChunkfiles    outputdir: $outputdir\n" if $DEBUG;

	#### SET CHUNKFILE
	my ($filestub, $suffix) = $inputfile =~ /\/([^\/]+)\.([^\.]+)$/;
	print "SNP::getChunkfiles    filestub: $filestub\n" if $DEBUG;
	print "SNP::getChunkfiles    suffix: $suffix\n" if $DEBUG;
	...
	my $binstart = $args->{binstart};
	my $binstop = $args->{binstop};
	print "SNP::getChunkfiles    chromosome: $chromosome\n" if $DEBUG;
	print "SNP::getChunkfiles    binstart: $binstart\n" if $DEBUG;
	print "SNP::getChunkfiles    binstop: $binstop\n" if $DEBUG;

	my $jobs = [];
	my $chunkfiles = [];
	my $chunknumber = 1;
	my $chunkfile = "$outputdir/$filestub.chunk$chunknumber.$suffix";
	print "SNP::getChunkfiles    chunkfile: $chunkfile\n" if $DEBUG;
	my $start = 0;
	my $stop = 0;
	while ( $stop < $binstop )	
	{
		...
		#my $commands = [];
		#my $command = "$samtools/samtools view -h -b $inputfile $region > $chunkfile";
		#print "SNP::getChunkfiles    $command\n" if $DEBUG;
		push @$chunkfiles, $chunkfile;
...
#	print "SNP::getChunkfiles    chunkfiles:\n" if $DEBUG;
#	print join "\n", @$chunkfiles;
#	print "\n";
#exit;
	...
	##### RUN JOBS
	#print "\nSNP::getChunkfiles    Running jobs\n";
...
my $DEBUG = 1;
	print "SNP::getNumberChunks    SAV::getNumberChunks(snpfile, chunksize)\n" if $DEBUG;
	print "SNP::getNumberChunks    chunksize: $chunksize\n" if $DEBUG;
	print "SNP::getNumberChunks    bin: \n" if $DEBUG;
	print Dumper $bin if $DEBUG;
	
	my $region = $bin->{stop} - $bin->{start};	
	my $numberchunks = int($region/$chunksize);
	$numberchunks++ if $region % $chunksize != 0;
	print "SNP::getNumberChunks    numberchunks: $numberchunks\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "SNP::chunkCommand    SNP::chunkCommand(args, chunknumber, inputfile, chunkdir)\n" if $DEBUG;	
	print "SNP::chunkCommand    chunknumber: $chunknumber\n" if $DEBUG;
	print "SNP::chunkCommand    inputfile: $inputfile\n" if $DEBUG;
	print "SNP::chunkCommand    chunkdir: $chunkdir\n" if $DEBUG;
	#print "SNP::chunkCommand    args: \n" if $DEBUG;
	#print Dumper $args;
	...
	print "SNP::chunkCommand    filename: $filename\n" if $DEBUG;
	print "SNP::chunkCommand    samtools: $samtools\n" if $DEBUG;
	print "SNP::chunkCommand    samtools_index: $samtools_index\n" if $DEBUG;
	...
	print "SNP::chunkCommand    predict: $predict\n" if $DEBUG;
	...
	#my $main_command = join ";\n", @$commands;
	#print "SNP::chunkCommand    main_command: $main_command\n";
	...
	print "SNP::chunkCommand    Returning snpfile: $snpfile\n" if $DEBUG;
	print "SNP::chunkCommand    Returning job: $job\n" if $DEBUG;
...
my $DEBUG = 1;
	print "SNP::indexBam    SNP::indexBam(chromosome, inputdirs, outputdir, filename, binlevel)\n" if $DEBUG;
	print "SNP::indexBam    inputdirs: @$inputdirs\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		foreach my $chromosome ( @$chromosomes )
		{
			...
			#print "SNP::indexBam    inputfile: $inputfile\n" if $DEBUG;
			#print "SNP::indexBam    outdir: $outdir\n" if $DEBUG;
...
my $DEBUG = 1;
	print "SNP::sortBam    SNP::sortBam(chromosome, inputdirs, outputdir, filename, binlevel)\n" if $DEBUG;
	print "SNP::sortBam    inputdirs: @$inputdirs\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		foreach my $chromosome ( @$chromosomes )
		{
			...
			my $inputfile = "$inputdir/$chromosome/$filename";
			my $outdir = "$outputdir/$chromosome";
			#print "SNP::sortBam    inputfile: $inputfile\n" if $DEBUG;
			#print "SNP::sortBam    outdir: $outdir\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "SNP::snpCounts    SNP::snpCounts(chromosome, inputdirs, outputdir, filename, binlevel)\n" if $DEBUG;
	#print "SNP::snpCounts    inputdirs: @$inputdirs\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		{	
			...
			my ($dirname) = $inputdir =~ /([^\/]+)$/;
			my $outdir = "$outputdir/$chromosome";
			#print "SNP::snpCounts    outdir: $outdir\n" if $DEBUG;
			File::Path::mkpath($outdir) if not -d $outdir;
			print "Can't create outdir: $outdir\n" if not -d $outdir;
			...
			foreach my $bin ( @$bins )
			{
				...
				#### DEBUG
				print "SNP::snpCounts    binfile: $binfile\n" if $DEBUG;
				print "SNP::snpCounts    idxstatsfile: $idxstatsfile\n" if $DEBUG;
				print "SNP::snpCounts    snpfile: $snpfile\n" if $DEBUG;
				print "SNP::savCounts    savfile: $savfile\n" if $DEBUG;
...
my $DEBUG = 1;
	print "SNP::bamToSnp    SNP::bamToSnp()\n" if $DEBUG;
...
my $DEBUG = 1;
	print "SNP::bamSnps    SNP::bamSnps(chromosome, inputdirs, outputdir, filename, zipped)\n" if $DEBUG;
	print "SNP::bamSnps    inputdirs: @$inputdirs\n" if $DEBUG;

	#### GET REQUIRED VARIABLES
	my $start 	=	$self->get_start();
	my $samtools = $self->get_samtools();
	my $samtools_index = $self->get_samtoolsindex();
	print "SNP::bamSnps    start: $start\n" if defined $start and $DEBUG;
	print "SNP::bamSnps    samtools: $samtools\n" if $DEBUG;
	print "SNP::bamSnps    samtools_index: $samtools_index\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		foreach my $chromosome ( @$chromosomes )
		{
			...
			my $inputfile = "$inputdir/$chromosome/$filename";
			my $outdir = "$outputdir/$chromosome";
			print "SNP::bamSnps    inputfile: $inputfile\n" if $DEBUG;
			print "SNP::bamSnps    outdir: $outdir\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "SNP::printLineCounts    SNP::printLineCounts(chromosome, inputdirs, outputdir, filename, binlevel)\n" if $DEBUG;
	print "SNP::printLineCounts    suffix: $suffix\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		foreach my $chromosome ( @$chromosomes )
		{
			#### CREATE CHROMOSOME OUTDIR
			my $outdir = "$outputdir/$chromosome";
			print "SNP::printLineCounts    outdir: $outdir\n" if $DEBUG;
			File::Path::mkpath($outdir) if not -d $outdir;
			print "Can't create outdir: $outdir\n" if not -d $outdir;
			
			#### CHECK INPUT BAM FILE
			my $bamfile = "$inputdir/$chromosome/$filename";
			print "SNP::printLineCounts    bamfile: $bamfile\n" if $DEBUG;
			...
			my $rangefile = "$inputdir/$chromosome/hit.bam.range";
			$bins = $binner->setBinsByRange($bins, $rangefile);
			#print "SNP::printLineCounts    bins:\n" if $DEBUG;
			#print Dumper $bins if $DEBUG;
...
#my $DEBUG = 1;
	print "SNP::printBamIdxstats    SNP::printBamIdxstats(chromosome, inputdirs, outputdir, filename, binlevel)\n" if $DEBUG;
	#print "SNP::printBamIdxstats    inputdirs: @$inputdirs\n" if $DEBUG;
...
#	}
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		foreach my $chromosome ( @$chromosomes )
		{
			#### SET OUTDIR
			my $outdir = "$outputdir/$chromosome";
			print "SNP::printBamIdxstats    outdir: $outdir\n" if $DEBUG;
			
			#### CHECK INPUT BAM FILE
			my $bamfile = "$inputdir/$chromosome/$filename";
			print "SNP::unbinSnp    bamfile: $bamfile\n" if $DEBUG;
			...
			my $rangefile = "$inputdir/$chromosome/hit.bam.range";
			$bins = $binner->setBinsByRange($bins, $rangefile);
			#print "SNP::printBamIdxstats    bins:\n" if $DEBUG;
			#print Dumper $bins if $DEBUG;
...
my $DEBUG = 1;
	print "SNP::createIntervals    SNP::createIntervals(inputfile, outputfile, window)\n" if $DEBUG;
	...
	while ( <FILE> )
	{
		...
		{
			#print "$chromosome:$currentstart-$currentstop\n";
...
my $DEBUG = 1;
	print "SNP::createGatkIntervals    SNP::createGatkIntervals(chromosome, inputdirs, outputdir, filename, binlevel)\n" if $DEBUG;
	print "SNP::createGatkIntervals    inputdirs: @$inputdirs\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		{	
			...
			my ($dirname) = $inputdir =~ /([^\/]+)$/;
			my $outdir = "$outputdir/$dirname/$chromosome";
			print "SNP::createGatkIntervals    outdir: $outdir\n" if $DEBUG;
			File::Path::mkpath($outdir) if not -d $outdir;
			print "Can't create outdir: $outdir\n" if not -d $outdir;
			...
			#### GET ARRAY OF BINS
			my $bins = $binner->getBins($binlevel, $chromosome_size);
			print "SNP::createGatkIntervals    bins:\n" if $DEBUG;
			print Dumper $bins if $DEBUG;
			...
			foreach my $bin ( @$bins )
			{
				...
				print "SNP::createGatkIntervals    targetsfile: $targetsfile\n" if $DEBUG;
				my $outputfile = "$outdir/$filename";
				print "SNP::createGatkIntervals    outputfile: $outputfile\n" if $DEBUG;
...
my $DEBUG = 1;
	print "SNP::createIntervalFiles    SNP::createIntervalFiles(chromosome, inputdirs, outputdir, filename, binlevel)\n" if $DEBUG;
	#print "SNP::createIntervalFiles    inputdirs: @$inputdirs\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		{	
			...
			#print "SNP::createIntervalFiles    outdir: $outdir\n" if $DEBUG;
			File::Path::mkpath($outdir) if not -d $outdir;
			print "Can't create outdir: $outdir\n" if not -d $outdir;
			...
			#### GET ARRAY OF BINS
			my $bins = $binner->getBins($binlevel, $chromosome_size);
			#print "SNP::createIntervalFiles    bins:\n" if $DEBUG;
			#print Dumper $bins if $DEBUG;
			...
			foreach my $bin ( @$bins )
			{
				...
				#print "SNP::createIntervalFiles    intervalsfile: $intervalsfile\n" if $DEBUG;

				#### SET SNP FILE
				my $snpfile = "$outputdir/$chromosome/$filename";
				$snpfile =~ s/\.bam$//;
				$snpfile .= "-$dirname.snp";
				#print "SNP::createIntervalFiles    snpfile: $snpfile\n" if $DEBUG;
...
my $DEBUG = 1;
	print "SNP::printRangefiles    SNP::printRangefiles(chromosome, inputdirs, outputdir, filename, binlevel)\n" if $DEBUG;
	print "SNP::printRangefiles    inputdirs: @$inputdirs\n" if $DEBUG;
	...
	#print "SNP::printRangefiles    rangefile: $rangefile\n";
	
	#if ( not -f $rangefile )
	#{
		#print "SNP::printRangefiles    Printing rangefiles because can't find rangefile: $rangefile\n";
...
my $DEBUG = 1;
	print "SNP::realignBam    SNP::realignBam(chromosome, inputdirs, outputdir, filename, binlevel)\n" if $DEBUG;
	print "SNP::realignBam    inputdirs: @$inputdirs\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		{	
			...
			my ($dirname) = $inputdir =~ /([^\/]+)$/;
			my $outdir = "$outputdir/$dirname/$chromosome";
			print "SNP::realignBam    outdir: $outdir\n" if $DEBUG;
			File::Path::mkpath($outdir) if not -d $outdir;
			print "Can't create outdir: $outdir\n" if not -d $outdir;
			...
			foreach my $bin ( @$bins )
			{
				...
				print "SNP::realignBam    intervalsfile: $intervalsfile\n" if $DEBUG;
				my $outputfile = "$outdir/$filename";
				print "SNP::realignBam    outputfile: $outputfile\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "SNP::unbinSnp    SNP::unbinSnp(chromosome, inputdirs, outputdir, filename, binlevel)\n" if $DEBUG;
	print "SNP::unbinSnp    inputdirs: @$inputdirs\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		{	
			...
			#### CREATE CHROMOSOME OUTDIR
			my $outdir = "$outputdir/$chromosome";
			print "SNP::unbinSnp    outdir: $outdir\n" if $DEBUG;
			File::Path::mkpath($outdir) if not -d $outdir;
			print "Can't create outdir: $outdir\n" if not -d $outdir;
			
			#### SET OUTPUT FILE
			my $outputfile = "$outdir/hit-$dirname.snp";
			print "SNP::unbinSnp    outputfile: $outputfile\n" if $DEBUG;
...
sub mergeBinBam {
	...
	#### GET CHROMOSOMES
	my $inputdir = $$inputdirs[0];
	#print "SNP::binBamToSnp    inputdir: $inputdir\n";
	my $chromosomes = $self->getChromosomes($inputdir);

#my $DEBUG = 1;
	#print "SNP::mergeBinBam    SNP::mergeBinBam(chromosome, inputdirs, outputdir, filename, binlevel)\n" if $DEBUG;
	#print "SNP::mergeBinBam    inputdirs: @$inputdirs\n" if $DEBUG;

	#### GET REQUIRED VARIABLES
	my $start 	=	$self->get_start();
	my $samtools = $self->get_samtools();
	my $samtools_index = $self->get_samtoolsindex();
	#print "SNP::mergeBinBam    start: $start\n" if defined $start and $DEBUG;
	#print "SNP::mergeBinBam    samtools: $samtools\n" if $DEBUG;
	#print "SNP::mergeBinBam    samtools_index: $samtools_index\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		foreach my $chromosome ( @$chromosomes )
		{
			...
			my $inputfile = "$inputdir/$chromosome/$filename";
			my $outdir = "$outputdir/$chromosome";
			#print "SNP::mergeBinBam    inputfile: $inputfile\n" if $DEBUG;
			#print "SNP::mergeBinBam    outdir: $outdir\n" if $DEBUG;
...
my $DEBUG = 1;
	print "SNP::cumulativeBam    SNP::cumulativeBam()\n" if $DEBUG;
	my $binlevel		=	$self->get_binlevel();
...
my $DEBUG = 1;
	print "SNP::cumulativeSnp    SNP::cumulativeSnp()\n" if $DEBUG;
	my $binlevel		=	$self->get_binlevel();
...
my $DEBUG = 1;
	print "SNP::mergeBam    SNP::mergeBam(chromosome, inputdirs, outputdir, filename, zipped)\n" if $DEBUG;
	print "SNP::mergeBam    inputdirs: @$inputdirs\n" if $DEBUG;

	#### GET REQUIRED VARIABLES
	my $start 	=	$self->get_start();
	my $samtools = $self->get_samtools();
	my $samtools_index = $self->get_samtoolsindex();
	print "SNP::mergeBam    start: $start\n" if defined $start and $DEBUG;
	print "SNP::mergeBam    samtools: $samtools\n" if $DEBUG;
	print "SNP::mergeBam    samtools_index: $samtools_index\n" if $DEBUG;
	...
	foreach my $inputdir ( @$inputdirs )
	{
		...
		foreach my $chromosome ( @$chromosomes )
		{
			...
			my $inputfile = "$inputdir/$chromosome/$filename";
			my $outdir = "$outputdir/$chromosome";
			print "SNP::mergeBam    inputfile: $inputfile\n" if $DEBUG;
			print "SNP::mergeBam    outdir: $outdir\n" if $DEBUG;
