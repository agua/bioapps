...
=cut
	...
	print "Sampler::fixed_width(args)\n" if $DEBUG;
...
print Dumper $args;
	...
	foreach my $directory ( @$directories )
	{
		...
		#print "readstatsfile printed: $readstatsfile\n";
	}
...
sub printListFile {
	...
	print "Sampler::printListFile    \n\ninfofile: $infofile\n" if $DEBUG;
	print "Sampler::printListFile    inputfile: $inputfile\n" if $DEBUG;
	print "Sampler::printListFile    reads in file: $reads (", $reads/$total, " total)\n" if $DEBUG;
	print "Sampler::printListFile    reads (all N listfiles) per directory: $readsnumber\n" if $DEBUG;
	print "Sampler::printListFile    reads (per N listfile) for this file: $file_readsnumber\n" if $DEBUG;
	print "Sampler::printListFile    directory fraction (file reads/directory reads): $directory_fraction ($reads/$total)\n";
	print "Sampler::printListFile    file readsnumber: $file_readsnumber\n" if $DEBUG;
	...
	#	print "Sampler::printListFile    remainderfile: $remainderfile->{filename}\n";
	#}
	...
	#print "Sampler::printListFile    Listfiles:\n";
	foreach my $listfile ( @$listfiles )
	{
		print "Sampler::printListFile    $listfile->{filename}\n";
	}
	#print "Sampler::printListFile    $remainderfile->{filename}\n";
	...
	while ( $record < ($reads - $remainder) )
	{
		...
		#print "Sampler::printListFile    done: \n";print Dumper $done;
		#print "Sampler::printListFile    listfiles_copy\n"; print Dumper @$listfiles_copy;
...
sub readlists {
	...
	foreach my $infofile ( @$infofiles )
	{
		...
		if ( defined $paired )
		{
			#print "Doing paired check...\n";
			if ( $infofilename =~ /_2[^\/]+$/ )
			{
				#print "Mate read: $infofilename. Nexting...\n";
				#exit;
				next;
			}
		}
		...
		while ( $record < ($reads - $remainder) )
		{
			...
			while ( @$listfiles_copy and $record < ($reads - $remainder) )
			{
				...
				#print $$listfiles_copy[$rand]->{filename}, "\n";
				#print "filehandle:\n";
...
sub get_listfiles {
	...
	#print "Sampler::get_listfiles    listfiles:\n";
	#print join "\n", @$listfiles;
	#print "\n";
...
sub set_listfiles {
	...
	#print "Sampler::set_listfiles    Sampler::set_listfiles(inputfile, numberfiles)\n";
	#print "Sampler::set_listfiles    inputfile: $inputfile\n";
	#print "Sampler::set_listfiles    numberfiles: $numberfiles\n";
...
sub infofiles {
	...
	my $files = files($args);
	#print "Sampler::infofiles    files: @$files\n";
	...
	foreach my $file ( @$files )
	{
		...
		#print "Sampler::infofiles    infofile: $infofile\n";
...
#my $DEBUG = 1;
	...
	print "Sampler::files    Sampler::files(args)\n" if $DEBUG;
	print "Sampler::files    args:\n" if $DEBUG;
	use Data::Dumper;
	...
	#print "Sampler::files    dirs: \n" if $DEBUG;
	#print join "\n", @dirs if $DEBUG;
	#print "Sampler::files    \n" if $DEBUG;
	
	foreach my $dir ( @dirs )
	{
		#print "Sampler::files    dir: $dir\n" if $DEBUG;		
		my $dirfiles = Util::files($dir);
		#print "Sampler::files    dirfiles: @$dirfiles\n" if $DEBUG;
	...
	if ( defined $compress and $compress =~ /^gzip$/ )
	{
		print "Sampler::files    Sampler::files    compress is defined. Looking for fasta.gz files...\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "Sampler::filepaths    Sampler::filepaths( { inputdir, compress, mode} )\n" if $DEBUG;
	print "Sampler::filepaths    args: \n" if $DEBUG;
	print Dumper $args if $DEBUG;
...
#print "Sampler::subfiles    number_listfiles: $number_listfiles\n";
#print "Sampler::subfiles    listfiles:\n";
#print join "\n", @$listfiles;
#print "\n";
...
#exit;
	...
	{		
		...
		print "Sampler::subfiles    listfile: $listfile\n" if $DEBUG;
		print "Sampler::subfiles    inputfile: $inputfile\n" if $DEBUG;
		...
		while ( <LISTFILE> )
		{
			...
			print "Sampler::subfiles    offset: $_\n" if $DEBUG;
			print "Sampler::subfiles    record: $record\n" if $DEBUG;
...
#last;
		...
		print "Sampler::subfiles    OUTFILE file printed:\n$outputfile\n\n" if $DEBUG;
...
sub qual {
	...
	{		
		...
		#if ( -f $qualfile )
		#{
		#	print "qualfile already present: $qualfile\n";
		#	next;
		#}
		...
		print "tempfile printed: $tempfile\n" if $DEBUG;
		print "hostname: ";
		print `hostname`;
...
$DEBUG = 1;
	...
	foreach my $listfile ( @$listfiles )
	{
		...
		print "Sampler::fasta     inputfile: $inputfile\n" if $DEBUG;
		print "Sampler::fasta     listfile: $listfile\n" if $DEBUG;
		...
		print "Sampler::fasta     fastafile: $fastafile\n" if $DEBUG;
...
#	print "Sampler::symbolic2numeric(quality, ascii)\n";
#    print "Quality (symbolic): $quality\n";
    ...
    #print "Quality (numeric): $numeric_quality\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "Sampler::mergeFiles    Sampler::mergeFiles(args)\n" if $DEBUG;
	print "Sampler::mergeFiles    args: \n" if $DEBUG;
	print Dumper $args if $DEBUG;
	...
	if ( not defined $indices and not defined $maxindex )
	{
		...
		print "Sampler::mergeFiles    maxindex: $maxindex\n" if $DEBUG;
	}

	#### GENERATE LISTFILES FROM INDICES
	$indices = "1-$maxindex" if not defined $indices;
	print "Sampler::mergeFiles    indices: $indices\n" if $DEBUG;
	...
	foreach my $listfile_index ( @$listfiles ) 
	{
		...
		{
			print "Sampler::mergeFiles    listfile_index: $listfile_index\n" if $DEBUG;
			print "Sampler::mergeFiles    inputfile: $inputfile\n" if $DEBUG;
			my ($file, $subdir, $directory) = file_subdir("$inputfile");
			print "Sampler::mergeFiles    file: $file\n" if $DEBUG;

			my $smallfile = "$directory/$subdir/$file.$listfile_index-$maxindex.";
			$smallfile .= "reads-fastq";
			print "Sampler::mergeFiles    smallfile: $smallfile\n" if $DEBUG;
		...
		if ( defined $paired )
		{
			print "Sampler::mergeFiles    Doing paired ends...\n" if $DEBUG;
		...
		{
			print "Sampler::mergeFiles    Doing single reads...\n" if $DEBUG;
...
=cut
	...
	print "Sampler::get_splitfiles    Sampler::get_splitfiles(filename)\n" if $DEBUG;
    print "Sampler::get_splitfiles    filename: $filename\n" if $DEBUG;
...
=cut
	...
	print "Sampler::Sampler::set_splitfiles(inputfiles, lines, splitfile, outputdir, suffix)\n" if $DEBUG;
...
#my $DEBUG = 1;
	...
	print "Sampler::splitFiles    Sampler::splitFiles(inputfiles, matefiles, lines, splitfile, outputdir, suffix)\n" if $DEBUG;
	...
	print "\n" if $DEBUG;
	print "Sampler::splitFiles    infiles:\n" if $DEBUG;
	print "\n" if $DEBUG;
	print join "\n", @infiles if $DEBUG;
	print "\n" if $DEBUG;

	my @mates = split ",", $matefiles if defined $matefiles;
	print "Sampler::splitFiles    mates:\n" if $DEBUG;
	print "\n" if $DEBUG;
	print join "\n", @mates if $DEBUG;
	print "\n" if $DEBUG;
	...
	print "Sampler::splitFiles    BEFORE printSplitfiles(outputdir, \@inputfiles, lines, index, suffix, label)\n" if $DEBUG;
	my $entries = printSplitfiles($outputdir, \@infiles, $lines, $index, $suffix, $label);
	foreach my $entry ( @$entries )
	{
		print "$entry->{filenumber}\t$entry->{index}\t$entry->{outputfile}\n" if $DEBUG;
	...
	print "Sampler::splitFiles    entries:\n" if $DEBUG;
	print Dumper $entries if $DEBUG;
	...
	print "Sampler::splitFiles    Returning splitfiles:\n" if $DEBUG;
...
#my $DEBUG = 1;
	...
	print "Sampler::printSplitfiles    Sampler::printSplitfiles(outputdir, files, lines, index, suffix, label)\n" if $DEBUG;
	...
	print "Sampler::printSplitfiles    outputdir: $outputdir\n" if $DEBUG;
	print "Sampler::printSplitfiles    files: @$files\n" if $DEBUG;
	print "Sampler::printSplitfiles    lines: $lines\n" if $DEBUG;
	print "Sampler::printSplitfiles    index: $index\n" if $DEBUG;
	print "Sampler::printSplitfiles    suffix: $suffix\n" if $DEBUG;
	print "Sampler::printSplitfiles    label: $label\n" if $DEBUG;
	...
	foreach my $file ( @$files )
	{	
		print "Sampler::printSplitfiles    filename: $filename\n" if $DEBUG;
...
#my $DEBUG = 1;
#
#	print "Sampler::splitfileName(filename)\n" if $DEBUG;
#	print "subdir: $subdir\n" if $DEBUG;
#	print "filename: $filename\n" if $DEBUG;
...
#my $DEBUG = 1;
	...
	print "Sampler::replaceAtQual    Sampler::replaceAtQual(inputfile, outputfile)\n" if $DEBUG;
...
#exit;
	...
	#### OPEN FILES
	print "Sampler::replaceAtQual    Opening files...\n" if $DEBUG;
	open(FILE, $inputfile) or die "Can't open input file: $inputfile\n";
	open(OUTFILE, ">$outputfile") or die "Can't open output file: $outputfile\n";
	...
	while ( <FILE> )
	{
		...
		#print $sequence_header, $sequence, $quality_header, $quality if $DEBUG;
	...
	if ( $copyback )
	{
		print "Sampler::replaceAtQual    Doing copyback\n" if $DEBUG; 
		my $copy_command = "mv $outputfile $inputfile";
		print "Sampler::replaceAtQual    $copy_command\n" if $DEBUG;
		`$copy_command`;
	}
	
	print "Sampler::replaceAtQual    output file printed:\n$outputfile\n\n" if $DEBUG;
