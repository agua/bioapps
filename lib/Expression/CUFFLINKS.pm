...
=head2
		...
		PURPOSE
	        ...
	        WRAPPER SCRIPT FOR RUNNING CUFFLINKS EXPRESSION ANALYSIS
...
=head2
	...
	PURPOSE
		...
		RETURN AN ARRAY OF SAM FILES GENERATED BY CUFFLINKS ALIGNMENT 
...
#my $DEBUG = 1;
	...
	print "CUFFLINKS::run    CUFFLINKS::run()\n" if $DEBUG;
	...
	print "CUFFLINKS::run    Quitting because no files found in directory: $reference\n" and exit if scalar(@referencefiles) == 0;
	...
	#print "CUFFLINKS::run    referencefiles: @referencefiles\n";
	...
	#print "CUFFLINKS::run    splitfile: $splitfile\n" if $DEBUG;
	my $splitfiles = Sampler::get_splitfiles($splitfile);
	print "CUFFLINKS::run    No splitfiles produced. Quitting\n" and exit if not defined $splitfiles or scalar(@$splitfiles) == 0;
	print "CUFFLINKS::run    length splitfiles: ", scalar(@$splitfiles), "\n" if $DEBUG;
	...
	print "CUFFLINKS::run    label: $label\n" if defined $label and $DEBUG;
	...
	if ( defined $chunks )
	{
		...
		foreach my $referencefile ( @referencefiles )
		{
			...
			#print "CUFFLINKS::run    bamfiles: @$bamfiles\n";
			...
			#print "CUFFLINKS::run    sort command:\n\n$sort_command\n\n";
	...
	#print "CUFFLINKS    mergebam_jobs:\n";
	#print Dumper $mergebam_jobs;
	
		#### RUN MERGE JOBS
		$self->runJobs($mergebam_jobs, 'MERGEBAM');
	
		$duration = $self->lastDuration();
		print "CUFFLINKS::run    ", Timer::current_datetime(), "    $duration    Completed merging BAM files\n" if $DEBUG;
		...
		foreach my $referencefile ( @referencefiles )
		{
			...
			print "CUFFLINKS::run    bam2sam command:\n\n$command\n\n" if $DEBUG;
	
			my $label = "bam2sam-$referencefile";
			my ($outputdir) = $bamfile =~ /^(.+)\/[^\/]+$/; 
			print "CUFFLINKS::run    label: $label\n" if $DEBUG;
			print "CUFFLINKS::run    outputdir: $outputdir\n" if $DEBUG;
	...
	#### UNCOMMENT
		...
		#### RUN CONVERSION JOBS
		#print "CUFFLINKS::run    DOING bam2sam conversion...\n";
		$self->runJobs($bam2sam_jobs);	
		#print "CUFFLINKS::run    FINISHED bam2sam conversion.\n";
		
		$duration = $self->lastDuration();
		print "CUFFLINKS::run    ", Timer::current_datetime(), "    $duration    Completed converting from BAM to SAM files\n";
	

		#### RUN CUFFLINKS ON THE MERGED SAM FILE FOR EACH REFERENCE
		#### TO GET EXPRESSION CALCULATIONS.
		print "CUFFLINKS::run    Before run CUFFLINKS\n";
		print "CUFFLINKS::run    outputdir: $outputdir\n";
		
		my $cufflinks_jobs = [];
		foreach my $referencefile ( @referencefiles )
		{
			print "CUFFLINKS::run    referencefile: $referencefile\n" if $DEBUG;
	
			#### NAME OUTPUT DIR AFTER REFERENCE FILE
			my $samfile = "$outputdir/$referencefile/accepted_hits.sam";
			print "CUFFLINKS::run    samfile: $samfile\n" if $DEBUG;
	
			my $commands = [];
			my $command = $self->cufflinksCommand($samfile);
			push @$commands, $command;
			print "CUFFLINKS::run    commands: @$commands\n" if $DEBUG;
			
			my $label = "cufflinks-$referencefile";
			my $outdir = "$outputdir/$referencefile";
			print "CUFFLINKS::run    label: $label\n" if $DEBUG;
			print "CUFFLINKS::run    outputdir: $outdir\n" if $DEBUG;
		...
		#### RUN CUFFLINKS
		$self->runJobs($cufflinks_jobs, 'CUFFLINKS');
		print "FINISHED RUNNING CUFFLINKS\n" if $DEBUG;
	
		$duration = $self->lastDuration();
		print "CUFFLINKS::run    ", Timer::current_datetime(), "    $duration    Completed cufflinks\n";

	}

	#### IF chunks IS NOT DEFINED, COPY SAM FILES AND RUN CUFFLINKS
	else
	{
		my $samtools = $self->get_samtools();
		my $samtools_index = $self->get_samtoolsindex();
		...
		foreach my $referencefile ( @referencefiles )
		{
			...
			print "CUFFLINKS::run    copysam command:\n\n$command\n\n" if $DEBUG;
	...
	#### UNCOMMENT
		...
		print "CUFFLINKS::run    DOING copysam...\n";	
		#### RUN CONVERSION JOBS
		$self->runJobs($copysam_jobs);
		print "CUFFLINKS::run    FINISHED copysam conversion.\n";
		...
		$duration = $self->lastDuration();
		print "CUFFLINKS::run    ", Timer::current_datetime(), "    $duration    Completed running Cufflinks\n";
	}
...
#my $DEBUG = 1;
	...
	print "CUFFLINKS::cufflinksCommand    CUFFLINKS::cufflinksCommand(samfile)\n" if $DEBUG;
	print "CUFFLINKS::cufflinksCommand    samfile: $samfile\n" if $DEBUG;
...
--num-threads $cpus \\
	...
	print "CUFFLINKS::cufflinksCommand    command: $command\n" if $DEBUG;
...
print "CUFFLINKS::new    CUFFLINKS::new(arguments)\n";
print "CUFFLINKS::new    arguments:\n";
...
#my $DEBUG = 1;
	print "CUFFLINKS::AUTOLOAD(self, newvalue)\n" if $DEBUG;
	print "New value: $newvalue\n" if $DEBUG and defined $newvalue;
    ...
    if($operation eq 'get') {
		...
		print "CUFFLINKS::AUTOLOAD    Returning attribute: $attribute\n" if $DEBUG;
    ...
    # AUTOLOAD mutators
        ...
        # define subroutine
#		print "CUFFLINKS::AUTOLOAD    Setting attribute: $attribute\n" if $DEBUG;
