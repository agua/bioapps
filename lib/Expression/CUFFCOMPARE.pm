...
#my $DEBUG = 1;
	...
	print "CUFFCOMPARE::run    CUFFCOMPARE::run()\n" if $DEBUG;
	...
	#print "CUFFCOMPARE::run    referencefiles: @referencefiles\n";
	...
	#print "CUFFCOMPARE::run    splitfile: $splitfile\n" if $DEBUG;
	...
	print "CUFFCOMPARE::run    length splitfiles: ", scalar(@$splitfiles), "\n" if $DEBUG;
	...
	print "CUFFCOMPARE::run    label: $label\n" if defined $label and $DEBUG;
	...
	if ( defined $chunks )
	{
		...
		foreach my $referencefile ( @referencefiles )
		{
			...
			#print "CUFFCOMPARE::run    bamfiles: @$bamfiles\n";
			...
			#print "CUFFCOMPARE::run    sort command:\n\n$sort_command\n\n";
	...
	#print "CUFFCOMPARE    mergebam_jobs:\n";
	#print Dumper $mergebam_jobs;
		...
		print "CUFFCOMPARE::run    ", Timer::current_datetime(), "    $duration    Completed merging BAM files\n" if $DEBUG;
		...
		foreach my $referencefile ( @referencefiles )
		{
			...
			print "CUFFCOMPARE::run    bam2sam command:\n\n$command\n\n" if $DEBUG;
	...
	#### UNCOMMENT
		...
		#### RUN CONVERSION JOBS
		#print "CUFFCOMPARE::run    DOING bam2sam conversion...\n";
		$self->runJobs($bam2sam_jobs);	
		#print "CUFFCOMPARE::run    FINISHED bam2sam conversion.\n";
		...
		foreach my $referencefile ( @referencefiles )
		{
			print "CUFFCOMPARE::run    referencefile: $referencefile\n" if $DEBUG;
	
			#### NAME OUTPUT DIR AFTER REFERENCE FILE
			my $samfile = "$outputdir/$referencefile/accepted_hits.sam";
			print "CUFFCOMPARE::run    samfile: $samfile\n" if $DEBUG;
	
			my $commands = [];
			my $command = $self->cuffcompareCommand($samfile);
			push @$commands, $command;
			print "CUFFCOMPARE::run    commands: @$commands\n" if $DEBUG;
			
			my $label = "cuffcompare-$referencefile";
			my $outdir = "$outputdir/$referencefile";
			print "CUFFCOMPARE::run    label: $label\n" if $DEBUG;
			print "CUFFCOMPARE::run    outputdir: $outdir\n" if $DEBUG;
		...
		print "FINISHED RUNNING CUFFCOMPARE\n" if $DEBUG;
	...
	else
	{
		...
		foreach my $referencefile ( @referencefiles )
		{
			...
			print "CUFFCOMPARE::run    copysam command:\n\n$command\n\n" if $DEBUG;
...
#my $DEBUG = 1;
	...
	print "CUFFCOMPARE::cuffcompareCommand    CUFFCOMPARE::cuffcompareCommand(samfile)\n" if $DEBUG;
	print "CUFFCOMPARE::cuffcompareCommand    samfile: $samfile\n" if $DEBUG;
...
/scratch/syoung/base/pipeline/bixby/run1/tophat/analysis10/chr1/transcripts.gtf
	...
	print "CUFFCOMPARE::cuffcompareCommand    command: $command\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "CUFFCOMPARE::AUTOLOAD(self, newvalue)\n" if $DEBUG;
    ...
    if($operation eq 'get') {
		...
		print "CUFFCOMPARE::AUTOLOAD    Returning attribute: $attribute\n" if $DEBUG;
    ...
    # AUTOLOAD mutators
        ...
        # define subroutine
#		print "CUFFCOMPARE::AUTOLOAD    Setting attribute: $attribute\n" if $DEBUG;
