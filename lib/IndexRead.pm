...
sub getValue
{
	...
	#print "IndexRead::printOut    IndexRead::printOut(db, key)\n";
	#print "IndexRead::printOut    db: \n";
	#$self->printOut($db);
	#print "IndexRead::printOut    key: $key\n" if defined $key;

	my $value;	
	#my $got = $db->get_dup($key);
	my $got = $db->get($key, $value);
	#print "IndexRead::getValue    got: $got\n";
	#print "IndexRead::getValue    value: $value\n";

	#my $status = $db->get_dup($key);
	#print "IndexRead::getValue    status: $status\n";
...
sub printOut
{
	...
	#print "IndexRead::printOut    counter: $counter\n";
}
...
#my $DEBUG = 1;
	...
	#print "IndexRead::compare   queryonlyfile already present: $queryonlyfile\n" and exit if -f $queryonlyfile;
	#print "IndexRead::compare   querybothfile already present: $querybothfile\n" and exit if -f $querybothfile;
	...
	#print "IndexRead::compare    queryhash:\n";
	#$self->printOut($querydb);

	#print "IndexRead::compare    targethash:\n";
	#$self->printOut($targetdb);


	### CHECK FOR MATCHING KEYS IN TARGET DATABASE
	print "IndexRead::compare   Counting max\n" if $DEBUG;
	my $total = 0;
	my $matched = 0;
...
#    {
		...
		if ( defined $found )	
		{
			#print "Target HAS key: $key\n";
			#print "Target HAS contain key: $key\t$value\n";
			$matched++;
		}
		else
		{
			#print "Target does not contain key: $key\t$value\n";
			#print "Target does not contain key: $key\n";
		}
	}
	#print "IndexRead::compare    total: $total\n" ;
...
#my $DEBUG = 1;
	#print "IndexRead::getReads    dbfile: $dbfile\n" if $DEBUG;
...
#    warn qx{ ps -o rss,vsz $$ }, "\n";
	...
	print "IndexRead::getReads    tying hash\n" if $DEBUG;
	my $hash;
	my $db = tie %$hash, "DB_File", $dbfile, O_RDWR|O_CREAT, 0666, $DB_BTREE 
		or die "Cannot open $dbfile: $!\n";
	print "IndexRead::getReads    hash tying completed\n" if $DEBUG;


	### PRINT KEY/VALUE PAIRS BY ITERATING THROUGH BTREE USING seq
	print "IndexRead::buildIndex   Counting max\n" if $DEBUG;
	my $total = 0;
	my ($key, $value, $status);
	$key = $value = 0 ;
    for ($status = $db->seq($key, $value, R_FIRST) ;
	...
	print "IndexRead::getReads    total: $total\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "IndexRead::mergeIndexes    dbfile: $outputfile\n" if $DEBUG;
	...
	#my $counter = 0;
	#my ($key, $value, $status);
	#print "IndexRead::mergeIndexes    Printing $max sample entries\n" if $DEBUG;
	#$key = $value = 0 ;
	#for ($status = $db->seq($key, $value, R_FIRST) ;
	#	 $status == 0 ;
	#	 $status = $db->seq($key, $value, R_NEXT) )
	...
	#	}
	#}
	#print "IndexRead::mergeIndexes    max: $counter\n" if $DEBUG;
	...
	foreach my $inputfile ( @$inputfiles )
	{
		...
		#	print "key $key entries :\n" if $DEBUG;
		#	print Dumper $entries if $DEBUG;
		#	
		#	print "key $key existing:\n" if %$existing and $DEBUG;
		#	print Dumper $existing if %$existing and $DEBUG;
		#	
		#	foreach my $entry ( @$entries )
		#	{
		...
		#	}
		#}
		#print "IndexRead::mergeIndexes    entries: $entries\n" if $DEBUG;
		#
		...
		#printf "IndexRead::mergeIndexes    Elapsed time (milliseconds)	\n", $milliseconds;
		...
		#	}
		#}
		#print "IndexRead::mergeIndexes    counter: $counter\n" if $DEBUG;
	...
	#$max = 100;
	#$counter = 0;
	#print "IndexRead::mergeIndexes    FINAL $max Printing sample entries\n" if $DEBUG;
	#$key = $value = 0 ;
	#for ($status = $db->seq($key, $value, R_FIRST) ;
	#	 $status == 0 ;
	#	 $status = $db->seq($key, $value, R_NEXT) )
...
sub numberReads
{
	...
	my $dbfile		=	$self->get_dbfile();
#my $DEBUG = 1;
	#print "IndexRead::numberReads    dbfile: $dbfile\n" if $DEBUG;
	...
	#### TIE HASH TO EXISTING DB FILE
	print "IndexRead::numberReads    tying hash\n" if $DEBUG;
	my $hash;
	my $db = tie %$hash, "DB_File", $dbfile, O_RDWR|O_CREAT, 0666, $DB_BTREE 
		or die "Cannot open $dbfile: $!\n";
	print "IndexRead::numberReads    hash tying completed\n" if $DEBUG;
	...
	#print "IndexRead::numberReads   db:\n" if $DEBUG;
	#$self->printOut($db);

	### PRINT KEY/VALUE PAIRS BY ITERATING THROUGH BTREE USING seq
	print "IndexRead::numberReads   Counting max\n" if $DEBUG;
	my $existing;
	my $total = 0;
	my ($key, $value, $status);
	$key = $value = 0 ;
	...
	#print "IndexRead::numberReads    total: $total\n" if $DEBUG;
...
#my $DEBUG = 1;
	#print "IndexRead::numberHits    dbfile: $dbfile\n" if $DEBUG;
...
#    $DB_BTREE->{'compare'} = \&Compare;
	...
	#### TIE HASH TO EXISTING DB FILE
	print "IndexRead::numberHits    tying hash\n" if $DEBUG;
	my $hash;
	my $db = tie %$hash, "DB_File", $dbfile, O_RDWR|O_CREAT, 0666, $DB_BTREE 
		or die "Cannot open $dbfile: $!\n";
	print "IndexRead::numberHits    hash tying completed\n" if $DEBUG;
...
#	### PRINT KEY/VALUE PAIRS BY ITERATING THROUGH BTREE USING seq
#	print "IndexRead::lookupRead   VALUES:\n";
#	$key = $value = 0 ;
#    for ($status = $db->seq($key, $value, R_FIRST) ;
#         $status == 0 ;
#         $status = $db->seq($key, $value, R_NEXT) )
#	{
#		print "IndexRead::lookupRead   $key -> $value\n"
#	}
	...
	if ( $mode eq "count" )
	{
		print "IndexRead::lookupRead   scalar = get_dup($readname, 1)\n" if $DEBUG;
		$output = $db->get_dup($readname, 1);
		print "$output\n" if $DEBUG;
	}
	elsif ( $mode eq "array" )
	{
		print "IndexRead::lookupRead   array = get_dup($readname):\n" if $DEBUG;
	...
	{
		print "IndexRead::lookupRead   hash = get_dup($readname, 1):\n" if $DEBUG;
		%$output = $db->get_dup($readname, 1);
		print "\n" if $DEBUG;
...
my $DEBUG = 1;
	print "IndexRead::buildIndex   IndexRead::buildIndex()\n" if $DEBUG;
	...
	foreach my $inputfile ( @$inputfiles )
	{
		#print "IndexRead::buildIndex    Loading inputfile: $inputfile\n";
		my $counter  = 0;
		my $total_reads  = 0;
		if ( $inputfile =~ /\.sam\.gz$/ )
		{
		...
		while ( <FILE> )
		{
			...
			#print "$id/$pair_order||$chromosome,$position,$flag\n";
		...
		#print "IndexReads::indexFile    Finished processing inputfile: $inputfile\n";
    }
...
#	print "IndexRead::numberReads    total: $total\n" if $DEBUG;
	...
	#my @keys = keys %$hash;
	#print "IndexReads::count of keys: ", $#keys + 1, "\n";
	...
	print "IndexRead::buildIndex    Reads loaded: $self->{_counter}\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "IndexRead::FlushWordCache    IndexRead::FlushWordCache()\n" if $DEBUG;
	#print "IndexRead::FlushWordCache    self->{_wordCache}:\n" if $DEBUG;
	#print Dumper $self->{_wordCache} if $DEBUG;
	...
	foreach my $key ( sort sort_ids keys %{$self->{_wordCache}} )
	{
		...
		#print "IndexRead::FlushWordCache    $id\t $entry\n" if $DEBUG;
    }

#	#### iterate through the btree using seq
#    #### and print each key/value pair.
#	print "IndexRead::FlushWordCache    iterate btree:\n" if $DEBUG;
#    my $key = 0;
#	my $value = 0 ;
#	my $status;
#	my $db = $self->{_db};
...
#	print "IndexRead::FlushWordCache    print hash:\n" if $DEBUG;
#	foreach ( keys %{$self->{_hash}} )
#    {
#		print "$_ \t$self->{_hash}->{$_}\n" if $DEBUG;
#	}

#print "IndexRead::FlushWordCache    Dump hash:\n" if $DEBUG;
...
#print "IndexRead::pairOrder    flag: $flag\n";
...
#print "IndexRead::pairOrder    pair_order: $pair_order\n";
	return $pair_order;
}
...
sub new
{
	...
	if ( defined $self->{_referencefile} )
	{
		...
		print "IndexRead::new    reference: $reference\n" if $DEBUG;
...
sub AUTOLOAD {
    ...
    # AUTOLOAD mutators
		...
		#print "shift->{attribute}: $self->{$attribute}\n";
