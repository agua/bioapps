...
sub stat_datetime_created {
	...
	#print "TOKENS: @$tokens\n";
	#print "STAT DATETIME: $stat_datetime\n";
...
#	print "STRING: $string\n";
	...
	while ( $string !~ /^\s*$/ )
	{
#		print "Do quote check\n";
		#print "Token counter: $token_counter\n";
		my $token = '';
		if ( $string =~ s/^\s*"// )
		{
			#print "STRING (inner) start: **$string**\n";
			while ( $string !~ /^\s*"/ and $string !~ /^\s*$/ )
			{
				$string =~ s/^\s*([^"^\s]+)//;
				($token) .= "$1 ";
				$token_counter++;	
#				print "Token counter (inner): $token_counter: $token\n";
			}
			$string =~ s/^\s*"//;
#			print "String at end (inner): $string\n";
		}
		else
		{
			$string =~ s/^\s*(\S+)\s*//;
			($token) .= $1;
#			print "No-quote token: $token\n";
		}
