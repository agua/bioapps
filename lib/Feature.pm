...
sub printOut {
	...
	$line .= "\n" if not $line =~ /\n$/;
	#print "Feature::printOut    line: $line\n";	
...
#my $DEBUG = 1;
	...
	print "Feature::setFiles    Feature::setFiles(args)\n" if $DEBUG;
	#print "self:\n";
	#print Dumper $self;
...
sub nextFeature {
	...
	while ( not defined $chromosome )
	{
		...
		$line = <$handle>;
		#print "Feature::nextFeature    line: $line\n";
...
#my $DEBUG = 1;
	print "Feature::upstream    Feature::upstream(other)\n" if $DEBUG;
	print "Feature::upstream    selfchromosome: $selfchromosome\n" if $DEBUG;
	print "Feature::upstream    otherchromosome: $otherchromosome\n" if $DEBUG;
	print "Feature::upstream    selfstart: $selfstart\n" if $DEBUG;
	print "Feature::upstream    otherstart: $otherstart\n" if $DEBUG;

	#### RETURN 1 IF TARGET ON UPSTREAM CHROMOSOME
	my $selfUpstream = $self->upstreamChromosome($selfchromosome, $otherchromosome);
	print "Feature::upstream    selfUpstream: $selfUpstream\n" if $DEBUG;
	...
	print "Feature::upstream    otherUpstream: $otherUpstream\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "Feature::upstreamChromosome    Feature::upstreamChromosome(selfchromosome, otherchromosome)\n" if $DEBUG;
	print "Feature::upstreamChromosome    selfchromosome: $selfchromosome\n" if $DEBUG;
	print "Feature::upstreamChromosome    otherchromosome: $otherchromosome\n" if $DEBUG;
	...
	print "Feature::upstreamChromosome    returning self_upstream: $self_upstream\n" if $DEBUG;
...
my $DEBUG = 1;
    print "Feature::scroll    Feature::scroll(other)\n" if $DEBUG;
    ...
    #print "Feature::scroll    chromosome: $chromosome, start: $start, stop: $stop\n" if $DEBUG;
    ...
    while ( $current->{start} < $start and $current->{stop} < $start )
    {
		...
		#print "Feature::scroll    current: \n";
		#print Dumper $current;
		return if not defined $current;
    }

    #print "Feature::scroll    line: $current->{line}\n" if $DEBUG;
    #print "Feature::scroll    toString", $self->toString(), "\n" if $DEBUG;
}
...
sub setMatcher {
	...
	#print "Feature::setMatcher    Feature::setMatcher(type, indexes)\n";
	#print "Feature::setMatcher    type: $type\n";
	#print "Feature::setMatcher    indexes: $indexes\n" if defined $indexes;
	...
	elsif ( $type =~ /^dbsnp$/i )
	{
		$subroutine = sub {
			return if not @_;
			my $input = shift;
			#print "Feature::setMatcher sub    dbsnp\n";
			#print "Feature::setMatcher sub    input: $input\n";
	
			$input =~ /^\S+\tchr(\S+)\t(\d+)\t(\d+)/;
			#print "Feature::setMatcher sub    \$1: $1, \$2: $2, \$3: $3\n";
...
#print "Feature::setIndexes()\n";

	my $indexes = $self->get_indexes();
#print "Feature::setIndexes    indexes:\n";
#print Dumper $indexes;
	...
	#print "chromosomeindex: $chromosomeindex\n";
...
#print "Feature::setFlanks()\n";

	my $flanks = $self->get_flanks();
#print "Feature::setFlanks    flanks: $flanks\n";
...
#my $DEBUG = 1;
	print "++++ Feature::AUTOLOAD(self, newvalue)\n" if $DEBUG;
    ...
    # AUTOLOAD mutators
     ...
     print "sETTING self->{$attribute} = $newvalue\n" if $DEBUG;
