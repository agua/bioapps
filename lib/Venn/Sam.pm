...
#my $DEBUG = 1;
	print "Venn::Sam    Venn::Sam(queryfile, targetfile)\n" if $DEBUG;
	print "Venn::Sam    queryfile: $queryfile\n" if $DEBUG;
	print "Venn::Sam    targetfile: $targetfile\n" if $DEBUG;
	print "Venn::Sam    outputdir: $outputdir\n" if $DEBUG;
	print "Venn::Sam    querylabel: $querylabel\n" if $DEBUG;
	print "Venn::Sam    targetlabel: $targetlabel\n" if $DEBUG;
...
sub setOutputFiles
{
	...
	#print "Venn::Sam::setOutputfiles    queryonly: $queryonly\n";
	#print "Venn::Sam::setOutputfiles    targetonly: $targetonly\n";
	#print "Venn::Sam::setOutputfiles    both: $both\n";
...
#my $DEBUG = 1;
print "Venn::Sam::match    Venn::Sam::match(queryfile, targetfile)\n" if $DEBUG;
print "Venn::Sam::match    queryfile: $queryfile\n" if $DEBUG;
print "Venn::Sam::match    targetfile: $targetfile\n" if $DEBUG;
	...
	while ( <QUERY> )
	{
		...
		my ($queryid) = $_ =~ /^(\S+)/;
		$queryid =~ s/#.+$//;
		print "Venn::Sam::match    queryid: $queryid\n" if $DEBUG;
		my $match = 0;
		foreach my $target ( @targets )
		{
			my ($targetid) = $target =~ /^(\S+)/;
			$targetid =~ s/#.+$//;
			print "Venn::Sam::match    targetid: $targetid\n" if $DEBUG;
			print "Venn::Sam::match    MATCHED\n" if $targetid eq $queryid and $DEBUG;
...
sub AUTOLOAD {
	...
	print "++++ Venn::Sam::AUTOLOAD(self, newvalue)\n" if $DEBUG;
