...
my $DEBUG = 1;
	print "Venn::Simple    Venn::Simple::simple(queryfile, targetfile, outputdir, suffix)\n" if $DEBUG;
	print "Venn::Simple    queryfile: $queryfile\n" if $DEBUG;
	print "Venn::Simple    targetfile: $targetfile\n" if $DEBUG;
	print "Venn::Simple    outputdir: $outputdir\n" if $DEBUG;
	print "Venn::Simple    suffix: $suffix\n" if $DEBUG;
...
print "keys[0]: $keys[0]\n";
	...
	print "Venn::Simple::simple    outputfiles:\n\n" if $DEBUG;
	print "$queryonly\n" if $DEBUG;
	print "$targetonly\n" if $DEBUG;
	print "$both\n" if $DEBUG;
	print "\n";

	print "queryonly: ", scalar(@$query_only_lines), "\n" if $DEBUG;
	print "targetonly: ", scalar(@$target_only_lines), "\n" if $DEBUG;
	print "both: ", scalar(@$both_lines), "\n" if $DEBUG;
...
my $DEBUG = 1;
	print "Venn::Simple    Venn::Simple(queryfile, targetfile, outputdir, suffix)\n" if $DEBUG;
	print "Venn::Simple    queryfile: $queryfile\n" if $DEBUG;
	print "Venn::Simple    targetfile: $targetfile\n" if $DEBUG;
	print "Venn::Simple    outputdir: $outputdir\n" if $DEBUG;
	print "Venn::Simple    suffix: $suffix\n" if $DEBUG;
	...
	print "Venn::Simple    querylabel: $querylabel\n" if $DEBUG;
	print "Venn::Simple    targetlabel: $targetlabel\n" if $DEBUG;
	...
	my $both;
	($queryonly, $targetonly, $both) = $self->setOutputFiles($outputdir, $querylabel, $targetlabel, $suffix);
	print "Venn::Simple::compare    queryonly: $queryonly\n" if $DEBUG;
	print "Venn::Simple::compare    targetonly: $targetonly\n" if $DEBUG;
	print "Venn::Simple::compare    both: $both\n" if $DEBUG;
...
#my $DEBUG = 1;
print "Venn::Simple::match    Venn::Simple::match(queryfile, targetfile)\n" if $DEBUG;
print "Venn::Simple::match    queryfile: $queryfile\n" if $DEBUG;
print "Venn::Simple::match    targetfile: $targetfile\n" if $DEBUG;
	...
	while ( <QUERY> )
	{
		...
		print "Venn::Simple::match    queryposition: $queryposition\n" if $DEBUG;
		my $match = 0;
		for ( my $i = 0; $i < @targets; $i++ )
		{
			...
			print "Venn::Simple::match    targetposition: $targetposition\n" if $DEBUG;
			print "Venn::Simple::match    MATCHED\n" if $targetposition eq $queryposition and $DEBUG;
...
sub AUTOLOAD {
	...
	print "++++ Venn::Simple::AUTOLOAD(self, newvalue)\n" if $DEBUG;
