...
sub uniqueHeterozygotes {
	...
	#my $DEBUG = 1;
	print "Hapmap::uniqueHeterozygotes    Hapmap::uniqueHeterozygotes()\n" if $DEBUG;
	print "Hapmap::uniqueHeterozygotes    inputfile: $inputfile\n" if $DEBUG;
	print "Hapmap::uniqueHeterozygotes    ids: $ids\n" if $DEBUG;
	...
	#print "Hapmap::uniqueHeterozygotes    idhash:\n" if $DEBUG;
	#print Dumper $idhash;
	...
	{
		print "Hapmap::uniqueHeterozygotes    Doing hapmaps for id: $id\n" if $DEBUG;
...
sub filterHeteros {
	...
	#my $DEBUG = 1;
	print "Hapmap::filterHeteros    Hapmap::filterHeteros()\n" if $DEBUG;
	#print "Hapmap::filterHeteros    lines: $$lines[1]\n" if $DEBUG;
	#print "Hapmap::filterHeteros    individual: $individual\n" if $DEBUG;
	#print "Hapmap::filterHeteros    idhash: \n" if $DEBUG;
	#print Dumper $idhash;
	#
	...
	foreach my $line ( @$lines )
	{
		...
		#print "Hapmap::uniqueHeterozygotes    id_genotypes: $id_genotypes\n";
		#print Dumper $id_genotypes;
		#print "elements: @elements\n";
		#print "Hapmap::uniqueHeterozygotes    idlist: @idlist\n";
		#print "Hapmap::uniqueHeterozygotes    genotypes: @genotypes\n";

		#### CHECK IF OUR INDIVIDUAL IS HETEROZYGOUS
		my $individual_genotype = $id_genotypes->{$individual};
		#print "Hapmap::uniqueHeterozygotes    individual_genotype: $individual_genotype\n" if defined $individual_genotype;
		
		#### WARN IF INDIVIDUAL DOES NOT HAVE A GENOTYPE.
		#### AND SKIP THIS LINE		
		#print "Hapmap::uniqueHeterozygotes    Skipping because individual genotype not defined\n" if not defined $individual_genotype;
		next if not defined $individual_genotype;
		
		#### SKIP IF HOMOZYGOTE
		my $isHomozygote = $self->isHomozygote($individual_genotype);
		#print "Hapmap::uniqueHeterozygotes    isHomozygote: $isHomozygote\n";
		...
		print "Hapmap::filterHeteros    heterozygote_found: $heterozygote_found\n" if $DEBUG;
...
sub isHomozygote {
	...
	#print "Hapmap::isHomozygote    genotype: $genotype\n";

	my ($a, $b) = split "", $genotype;
	
	#print "Hapmap::isHomozygote    returning 1\n" if $a eq $b;
	#print "Hapmap::isHomozygote    returning 0\n" if not $a eq $b;
...
#my $DEBUG = 1;
	print "Hapmap::new    Hapmap::new(arguments)\n" if $DEBUG;
	print "Hapmap::new    arguments:\n" if $DEBUG;
...
sub initialise
{
	...
	#my $DEBUG = 1;	
	print "Hapmap::initialise    Hapmap::initialise(arguments)\n" if $DEBUG;
