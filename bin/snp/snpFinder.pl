...
close(REF);
$reference =~ s/^>[^\n]+\n//;
#print "reference: ", substr($reference, 0, 10), "\n";
#print "length(reference): ", length($reference), "\n";
...
while ( <INFILE> )
{
	...
	while ( $reference =~ /($sequence)/g ) {
        ...
        #print "sequence $sequence matches $1 at position $position\n";
...
{	
	...
	#my $DEBUG = 1;
	print "snpFinder.pl::set_parameter    snpFinder.pl::set_parameter(arguments, parameter, value)\n" if $DEBUG;
	print "snpFinder.pl::set_parameter    parameter: $parameter\n" if $DEBUG;
	print "snpFinder.pl::set_parameter    value: $value\n" if $DEBUG;
...
{	
	...
	#my $DEBUG = 1;
	print "snpFinder.pl::fill_in    snpFinder.pl::fill_in(arguments, value)\n" if $DEBUG;
	print "snpFinder.pl::fill_in    arguments:\n" if $DEBUG;
	print join "\n", @$arguments if $DEBUG;
	print "\n";
	print "snpFinder.pl::fill_in    value: $value\n" if $DEBUG;
...
#my $DEBUG = 1;
print "snpFinder.pl::get_argument    snpFinder.pl::get_argument(arguments, name)\n" if $DEBUG;
print "snpFinder.pl::get_argument    arguments:\n" if $DEBUG;
print join "\n", @$arguments if $DEBUG;
print "\n";
print "snpFinder.pl::get_argument    name: $name\n" if $DEBUG;
