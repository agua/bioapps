...
=head2
    ...
    --label               :   Name given to submitted cluster jobs
    --keep                :   Keep intermediate files
    --cluster             :   Name of Cluster
	--username			  :   Username of cluster owner
    --maxjobs             :   Max. number of concurrent clustertype maxjobs (DEFAULT: 30)
...
my $tophat = $conf->getKey("applications:aquarius-8", 'TOPHAT');
my $bowtie = $conf->getKey("applications:aquarius-8", 'BOWTIE');
my $cufflinks = $conf->getKey("applications:aquarius-8", 'CUFFLINKS');
...
print "TOPHAT.pl    clustertype: $clustertype\n";
...
print "TOPHAT.pl    matefiles: $matefiles\n" if $DEBUG;

##### CHECK LENGTH OF ARRAYS MATCHES AND ORDER OF FILES 
#if ( defined $matefiles and $matefiles )
#{
#    print "TOPHAT.pl    Checking paired files match up.\n";
#    my $filetool = FileTools->new();
#    my $mates_paired = $filetool->matesPaired(\@ins, \@mates);
#    print "TOPHAT.pl    Paired files match\n" if $mates_paired;
#    print "TOPHAT.pl    Paired files do not match. Exiting\n" if not $mates_paired;
#}
