...
print "samToBam.pl    samtools index: $samtools_index\n" if $DEBUG;
...
#### DEBUG
print "samToBam.pl    inputfiles: $inputfiles\n" if $DEBUG;
print "samToBam.pl    outputfiles: $outputfiles\n" if defined $outputfiles and $DEBUG;
...
for ( my $i = 0; $i < @$samfiles; $i++ )
{
	...
	print "samToBam.pl    command: $command\n" if $DEBUG;
