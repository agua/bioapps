...
#my $DEBUG = 1;
print "averageQuality::bin_qualities    averageQuality::bin_qualities(outputfile, binarray, dot)\n" if $DEBUG;
print "averageQuality::bin_qualities    outputfile: $outputfile\n" if $DEBUG;
	...
	while ( defined ($avqual = <FILE>) and defined $avqual )
	{
		...
		print "averageQuality.pl    avqual: **$avqual**\n" if $DEBUG;
		#$avqual =~ s/\n$//;
		#print "averageQuality.pl    avqual: **$avqual**\n";
		
		my ($qual) = $avqual =~ /(\S+)\s*$/;
	
		print "averageQuality.pl    qual: $qual\n" if $DEBUG;
...
#my $DEBUG = 1;
	print "averageQuality.pl::average_qualities    average_qualities(inputfile, outputfile)\n" if $DEBUG;
	print "averageQuality.pl::average_qualities    inputfile: $inputfile\n" if $DEBUG;
	print "averageQuality.pl::average_qualities    outputfile: $outputfile\n" if $DEBUG;
	...
	{    
		...
		print "averageQuality.pl::average_qualities         record: $record\n" if $DEBUG;
		...
		print "averageQuality.pl::average_qualities         quality: $quality\n" if $DEBUG;
		print "averageQuality.pl::average_qualities         fasta: $fasta\n" if $DEBUG;
		my ($header) = $record =~ /^\@(.+)\n/;
		$header =~ s/\s+length=.+$//;
		print "averageQuality.pl::average_qualities         header: $header\n" if $DEBUG;
	
		my ($symbolic) = $quality =~ /(\S+)\s*$/;	
		my $numeric = $solexa->symbolic2numeric($symbolic);
		print "averageQuality.pl::average_qualities         numeric: $numeric\n" if $DEBUG;
		...
		if ( defined $skip )
		{
			for ( my $i = 0; $i < $skip; $i++ )
			{
				...
				print "averageQuality.pl::average_qualities         skip $skip $i discarded: $discarded\n" if $DEBUG;
			}
		}
		...
		if ( defined $length )
		{
			...
			while ( $#numbers >= $length )
			{
				...
				print "averageQuality.pl::average_qualities         length $length discarded: $discarded\n" if $DEBUG;
			}
		}
		...
		print "averageQuality.pl::average_qualities         average: $average\n" if $DEBUG;
	...
	print "averageQuality.pl::average_qualities    basequalfile: $basequalfile\n" if $DEBUG;
...
#my $DEBUG = 1;
	...
	print "averageQuality.pl    compress(filename, compress)\n" if $DEBUG;
	print "averageQuality.pl    filename: $filename\n" if $DEBUG;
	print "averageQuality.pl    compress: $compress\n" if $DEBUG;
...
{
	my $array	=	shift;
	#print "averageQuality::average    array: @$array\n";
