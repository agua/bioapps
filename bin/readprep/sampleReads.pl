...
sub samples
{
	...
	#}
	#print "Finished generating qual files\n";
	#
...
sub readlists
{
	...
	foreach my $infofile ( @$infofiles )
	{
		...
		{
			#print OUT $record_length * $record_index, "\n";
			my $location = $record_length * $record_index;
			print OUT $location, "\n";
			$record_index++;
		}
...
sub print_files
{
	print "++++ print_files(args, samples, type, ascii)\n" if $DEBUG;
		...
		for ( my $fileindex = 0; $fileindex < @$infofiles; $fileindex++ )
		{
			...
			while ( <LISTFILE> )
			{
				...
				if ( $type =~ /qual$/ )
				{
					...
					#print "output: $output\n";
					#print "id: $id\n";
					#print "symbolic: $symbolic_quality\n";
					my $numeric_quality = symbolic2numeric($symbolic_quality, $ascii);
					$output = "$id\n$numeric_quality\n";
					#print "numeric: $numeric_quality\n";
					#print "output: $output";
					#exit;
				}
...
sub listfiles
{
	#print "++++ listfiles(inputfile, numberfiles)\n" if $DEBUG;
...
#	print "++++ symbolic2numeric(quality, ascii)\n";
