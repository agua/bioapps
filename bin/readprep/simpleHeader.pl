...
=head2
    ...
    PURPOSE
		...
		ACCEPTED BY ELAND AND OTHER ALIGNERS MATCHING THIS REGEX:
			...
			(I.E.: basic-sequence-info#barcode/matenumber)
...
print "FileTools::simpleHeader    inputfile: $inputfile\n" if $DEBUG;
print "FileTools::simpleHeader    outputfile: $outputfile\n" if $DEBUG;

#### OPEN FILE AND SET RECORD SEPARATOR
$/ = "\n";
if( $inputfile =~ /\.gz$/ or $inputfile =~ /\.zip$/ )
{
	print "simpleHeader.pl    Opening with zcat inputfile: $inputfile\n" if $DEBUG;
	my $pipe_command = "zcat $inputfile |";
	open(FILE, $pipe_command) or die "Can't open inputfile: $inputfile\n"
}
else
{
	print "simpleHeader.pl    Opening normally inputfile: $inputfile\n" if $DEBUG;
...
while ( <FILE> )
{
	...
	{
		my $sequence_header = "$1:$2";
		#print "simpleHeader.pl    sequence header: $sequence_header\n";
...
#my $DEBUG = 1;
	...
	print "simpleHeader.pl    compress(filename, compress)\n" if $DEBUG;
	print "simpleHeader.pl    filename: $filename\n" if $DEBUG;
	print "simpleHeader.pl    compress: $compress\n" if $DEBUG;
