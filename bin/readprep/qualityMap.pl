...
while ( defined ($avqual = <FILE>) and defined $avqual )
{
	...
	print "qualityMap.pl    avqual: **$avqual**\n" if $DEBUG;
	#$avqual =~ s/\n$//;
	#print "qualityMap.pl    avqual: **$avqual**\n";
	
	my ($qual) = $avqual =~ /(\S+)\s*$/;

	print "qualityMap.pl    qual: $qual\n" if $DEBUG;
...
my $DEBUG = 1;
	print "qualityMap.pl    compress(filename, compress)\n" if $DEBUG;
	print "qualityMap.pl    filename: $filename\n" if $DEBUG;
	print "qualityMap.pl    compress: $compress\n" if $DEBUG;
...
{
	my $array	=	shift;
	#print "qualityMap::average    array: @$array\n";
