...
=head2
    ...
    OUTPUT
        ...
        1. readFilter OUTPUT FILES IN ASSEMBLY DIRECTORY
...
my @arguments = @ARGV;
#print "readFilter.pl    Arguments: @arguments\n";
...
#
	...
	#print "readFilter.pl    splitfile: $splitfile\n" if $DEBUG;
	...
	print "readFilter.pl::run    length splitfiles: ", scalar(@$splitfiles), "\n" if $DEBUG;
	#print "readFilter.pl    splitfiles: \n" if $DEBUG;
...
#exit;
	...
	for my $referencefile ( @referencefiles )
	{
		...
		$referencefile = "$referencedir/$referencefile";
		#print "readFilter.pl    referencefile: $referencefile\n" if $DEBUG;
...
#	$current_time = time();
#	print "readFilter.pl    ", Timer::current_datetime(), "    $durations->{alignment}    Completed alignments";
#
#
#	
#	###############################################
...
#	$current_time = time();
#	print "readFilter.pl    ", Timer::current_datetime(), "    $durations->{maq2sam}    Completed alignments";
#
	...
	print "readFilter.pl    DOING snpCommands\n" if $DEBUG;
	my $snp_jobs = [];
	for my $referencefile ( @referencefiles )
	{
		next if $referencefile =~ /^[\.]+$/;
...
my $DEBUG = 0;
	...
	print "readFilter.pl::statValue    statValue(stats,field)\n" if $DEBUG;
	print "readFilter.pl::statValue    length stats: ", scalar(@$stats), "\n" if $DEBUG;
	print "readFilter.pl::statValue    field: $field\n" if $DEBUG;
	print "readFilter.pl::statValue    operation: $operation\n" if $DEBUG;
	
	return if not defined $stats or not @$stats;

	#print "readFilter.pl::statValue    stats[0]->{$field}:\n";
	#print Dumper $$stats[0]->{$field};
	#print "\n";
	
	my ($suffix) = $$stats[0]->{$field} =~ /^\s*[\d\.]+\s+(\S+)$/;
	$suffix = '' if not defined $suffix;
	print "readFilter.pl::statValue    suffix: $suffix\n" if $DEBUG;
	...
	foreach my $stat ( @$stats )
	{
		...
		#print "readFilter.pl::statValue    stat:\n";
		#print Dumper $stat;
	...
	print "readFilter.pl::statValue    value: $value\n" if $DEBUG;
...
my $DEBUG = 1;
	...
	print "readFilter.pl::printUsage    printUsage(usagefile, total_usage, durations, arguments)\n" if $DEBUG;
