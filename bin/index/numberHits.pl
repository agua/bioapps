...
=head2
    ...
    ./numberHits.pl <--inputfile String> [--help]
...
/nethome/bioinfo/apps/agua/0.5/bin/apps/index/numberHits.pl \
...
{
    print "numberHits.pl    Use option --help for usage instructions.\n";  exit;    };

#### PRINT HELP
if ( defined $help )	{	usage();	}

#### CHECK INPUTS
print "numberHits.pl    dbfile not defined (use --help option)\n" and exit if not defined $dbfile;
print "numberHits.pl    Can't find dbfile: $dbfile\n" and exit if not -f $dbfile;
...
my $output = $indexer->numberHits($readname, $mode);
print "$output\n";
