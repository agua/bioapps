...
my @arguments = @ARGV;
#print "insertSize.pl    arguments: @arguments\n";
...
while ( defined $readline and $reads < $lines )
{
	...
	if ( defined $match and $match !~ /NM\s*/ )
	{
		...
		my $x = $array[4];
		my $y = $array[5];
		#print "insertSize.pl    readline: $readline\n";
		#print "insertSize.pl    id: $id\n";
		#print "insertSize.pl    chromosome: **$chromosome**\n";
		#print "insertSize.pl    position: $position\n";

		my $mate = $mates->{"$id$machine$lane$x$y"};
		#print Dumper $mate if defined $mate;
		#print "insertSize.pl    mate NOT FOUND\n" if not defined $mate;
		if ( defined $mate and $chromosome eq $mate->{chromosome} )
		{
			#print "insertSize.pl    mate FOUND for read: $readline\n" if not defined $mate;
			my $distance = abs($position - $mate->{position});
			#print "insertSize.pl    distance: $distance\n";
